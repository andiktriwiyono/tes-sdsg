...
function createStudentCard(student) {
  ...
  if (isInTestPool && currentUser && currentUser.role === 'KOORDINATOR_RUANG_TEST') {
    ...
    if (isInTop5) {
      // Check which tables are occupied (siswa sudah di meja)
      ...
      // Check which tables are already plotted (siswa sudah di-plot tapi belum diantar)
      const table1Plotted = studentsData.some(s => s.plot_meja === 'meja-1' && s.id !== student.id);
      const table2Plotted = studentsData.some(s => s.plot_meja === 'meja-2' && s.id !== student.id);
      const table3Plotted = studentsData.some(s => s.plot_meja === 'meja-3' && s.id !== student.id);
      const table4Plotted = studentsData.some(s => s.plot_meja === 'meja-4' && s.id !== student.id);
      const table5Plotted = studentsData.some(s => s.plot_meja === 'meja-5' && s.id !== student.id);
      ...
      // Count available tables (not occupied AND not plotted AND not on break)
      const availableTables = [
        !table1Occupied && !table1Plotted && !table1Break,
        !table2Occupied && !table2Plotted && !table2Break,
        !table3Occupied && !table3Plotted && !table3Break,
        !table4Occupied && !table4Plotted && !table4Break,
        !table5Occupied && !table5Plotted && !table5Break
      ].filter(Boolean).length;
      
      // Generate buttons (disabled if table occupied OR plotted OR on break)
      const btn1 = table1Occupied 
        ...
        : table1Plotted
        ? `<button class="bg-blue-300 text-blue-700 text-xs py-1 px-1 rounded cursor-not-allowed font-medium" disabled title="Meja 1 sudah di-plot">📍</button>`
        : table1Break
      ...
      const btn2 = table2Occupied 
        ...
        : table2Plotted
        ? `<button class="bg-blue-300 text-blue-700 text-xs py-1 px-1 rounded cursor-not-allowed font-medium" disabled title="Meja 2 sudah di-plot">📍</button>`
        : table2Break
      ...
      const btn3 = table3Occupied 
        ...
        : table3Plotted
        ? `<button class="bg-blue-300 text-blue-700 text-xs py-1 px-1 rounded cursor-not-allowed font-medium" disabled title="Meja 3 sudah di-plot">📍</button>`
        : table3Break
      ...
      const btn4 = table4Occupied 
        ...
        : table4Plotted
        ? `<button class="bg-blue-300 text-blue-700 text-xs py-1 px-1 rounded cursor-not-allowed font-medium" disabled title="Meja 4 sudah di-plot">📍</button>`
        : table4Break
      ...
      const btn5 = table5Occupied 
        ...
        : table5Plotted
        ? `<button class="bg-blue-300 text-blue-700 text-xs py-1 px-1 rounded cursor-not-allowed font-medium" disabled title="Meja 5 sudah di-plot">📍</button>`
        : table5Break
      ...
      if (availableTables > 0) {
        // Show buttons for all 5 tables (disabled if occupied or plotted or on break)
        claimButtons = `
          <div class="mt-2 p-2 bg-green-50 border border-green-200 rounded">
            <p class="text-xs text-green-700 font-semibold mb-1">🔔 PLOT SISWA INI!</p>
            <p class="text-xs text-green-600 mb-2">Antrian ${queuePosition} - Pilih meja tersedia (${availableTables}/5):</p>
        ...
        // All tables occupied or plotted or on break
        ...
        const plottedCount = [table1Plotted, table2Plotted, table3Plotted, table4Plotted, table5Plotted].filter(Boolean).length;
        
        let statusMessage = '';
        if (plottedCount > 0) {
          statusMessage = `${occupiedCount} terisi, ${plottedCount} di-plot, ${breakCount} istirahat`;
        } else if (breakCount > 0 && occupiedCount > 0) {
      ...
      claimButtons = `
        <div class="mt-2 p-2 bg-amber-50 border border-amber-200 rounded">
          ...
          <p class="text-xs text-amber-600 mt-1">Tunggu 5 siswa pertama di-plot dulu</p>
        </div>
      `;
    }
  }
